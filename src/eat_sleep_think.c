...
/*   Updated: 2023/10/01 17:34:00 by vcacador         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	think(int i)
{
	...
	if (utils()->n_f % 2 != 0)
	{
		if (((philo()[0][i].last_meal - time_now()) + (uint64_t)utils()->tte) > (uint64_t)utils()->ttd && (((uint64_t)utils()->tte) * 3 > (uint64_t)utils()->ttd))
		{	
			printf("time -> %ld\n", (uint64_t)utils()->ttd - time_now() + 5);
	...
	{
		if (((philo()[0][i].last_meal - time_now()) + (uint64_t)utils()->tte) > (uint64_t)utils()->ttd && (((uint64_t)utils()->tte) * 2 > (uint64_t)utils()->ttd))
		{	
			printf("time -> %ld\n", (uint64_t)utils()->ttd - time_now() + 5);
			if (sleep_op((uint64_t)utils()->ttd - time_now() + 5, i))
				return (1);
